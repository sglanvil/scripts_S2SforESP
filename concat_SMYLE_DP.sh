...
# location of this file: /glade/work/sglanvil/CCR/yeager
...
for imember in {011..030}; do
        ...
        echo ${case}
        dirSMYLE=/glade/campaign/cesm/development/espwg/SMYLE/archive/${case}/lnd/proc/tseries/month_1/
        dirDP=/glade/campaign/cesm/development/espwg/CESM2-DP/timeseries/${case}/lnd/proc/tseries/month_1/
        if [ ! -d ${dirSMYLE} ] || [ ! -d ${dirDP} ]; then
                echo ${iyear} ${imember} "directory does not exist <skipping> - SASHA"
                continue
        fi
        echo ${iyear} ${imember}
        file1=$(ls ${dirSMYLE}*.h0.${ivar}.${iyear}*)
        file2=$(ls ${dirDP}*.h0.${ivar}.$((iyear+2))*)
        ...
        destDirNew=${destDir}/b.e21.BSMYLE.f09_g17.${iyear}-${imonth}.${imember}/lnd/proc/tseries/month_1/
        mkdir -p ${destDirNew}
